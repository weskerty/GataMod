...
const savePath = path.join('../tmp');  // Donde descarga el archivo temporal.
const downloadingFiles = new Set(); 
let browserInstance = null; 
let activeDownloads = 0;  // Contador para las descargas activas, todavia no limita.
const maxDownloads = 1; // Máximo de descargas simultáneas, para evitar saturacion del server.
const queue = [];
...
async function handleDownloadRequest(url, conn, m) {
  ...
  try {
    ...
    console.log(`MHTML enviado exitosamente.`);
